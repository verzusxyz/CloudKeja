                ...
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div>
                        <div class="text-end">
                            ...
                            <p class="info-p"> {{__('Name:')}} <span>{{ $subscriber->user?->name }}</span></p>
                            <p class="info-p pt-2"> {{__('Address:')}}
                                <span>{{ collect($subscriber->user?->user_details['address_info'] ?? [])->only(['address_one', 'city'])->filter()->implode(', ') }}</span>
                            </p>
                            <p class="info-p pt-2"> {{__('Phone:')}} <span>{{ $subscriber->user?->phone['mobile_no'] ?? 'N/A' }}</span>
                            </p>
                            <p class="info-p pt-2"> {{__('Email:')}} <span>{{ $subscriber->user?->email ?? 'N/A' }}</span></p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-bordered invoice-table-new">
                            ...
                            <tbody class="in-table-body-container">
                                <tr class="in-table-body">
                                    ...
                                    <td class="text-center">{{ $subscriber->plan_name ?? 'N/A' }}</td>
                            ...
                            <table class="table">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="border-0 text-end">{{ currency_format($subscriber->price ?? 0) }}</td>
                                    </tr>
                                    <tr>
