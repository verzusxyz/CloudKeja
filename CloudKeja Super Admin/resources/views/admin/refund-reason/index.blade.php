        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="woodland-card-header">
                    <h3>{{ __('Refund Reason List') }}</h3>
                    @can('refund_reasons-create')
                    <a href="" data-bs-toggle="modal" data-bs-target="#add-reason-modal" class="modal-btn"><i class="fal fa-plus-circle"></i>{{ __('Add Reason') }}</a>
                    @endcan
                </div>
                <div class="woodland-card">
                    ...
                    <div class="mb-3 d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <div>
                            <form action="{{ route('admin.refund-reason.filter') }}" method="post"
                                class="filter-form form-inline d-flex align-items-center gap-3 flex-wrap"
                                table="#refund-reason-data">
                                @csrf
                    ...
                    <div class="delete-item delete-show d-none">
                        <div class="delete-item-show">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                                data-url="{{ route('admin.refund-reason.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    ...
                    <div class="responsibe-table">
                        <table class="table woodland-table">
                            ...
                            <tbody id="refund-reason-data">
                                @include('admin.refund-reason.datas')
                            </tbody>
                        </table>
                    </div>
                    <div class="woodladn-pagination-wrapper">
                        <nav class="woodland-pagination">
                            <ul class="pagination">
                                <li class="page-item">{{ $reasons->links() }}</li>
                            </ul>
                        </nav>
                    </div>
                </div>
...
@include('admin.refund-reason.create')
@include('admin.refund-reason.edit')
@endpush
