        ...
        <td class="text-start w-150">
            <label class="switch">
                <input type="checkbox" {{ $category->status == 1 ? 'checked' : '' }} class="status"
                    data-url="{{ route('landlord.expense-categories.status', $category->id) }}">
            ...
            <div class="d-flex align-items-center justify-content-center action-container gap-2">
                <div>
                    <a href="#edit-expense-categories-modal" data-bs-toggle="modal" class="expense-category-edit-btn edit-action d-flex align-items-center justify-content-center"
                        data-url="{{ route('landlord.expense-categories.update', $category->id) }}" data-expense-category-name="{{ $category->name }}" data-expense-category-description="{{ $category->description }}"
                        >
                        <img class="" src="{{ asset('backend/assets/img/icon/edit.svg') }}">
                    </a>
                </div>
                <div>
                    <a href="{{ route('landlord.expense-categories.destroy', $category->id) }}" class="confirm-action delete-action d-flex align-items-center justify-content-center"
                        data-method="DELETE">
