        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="woodland-card-header">
                    <h3>{{ __('User Method List') }} </h3>
                    <div class="action-btn-wrapper">
                        <a data-bs-toggle="modal" data-bs-target="#add-bank-modal"
                            class="cursor-pointer d-print-none modal-btn"><i class="fal fa-plus-circle"></i>
                            {{ __('Add New Method') }}</a>
                    </div>
                </div>
                <div class="woodland-card">
                    ...
                    <div class="mb-3 d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <div>
                            <form action="{{ route('landlord.withdraw-method.filter') }}" method="post"
                                class="filter-form form-inline d-flex align-items-center gap-3 flex-wrap"
                                table="#deposit-utilities-datas">
                                @csrf
                        ...
                        <div class="d-flex align-items-center gap-3 d-print-none">
                            <form action="{{ route('landlord.withdraw-method.filter') }}" method="post" class="filter-form"
                                table="#deposit-utilities-datas">
                                @csrf
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu export-dropdown">
                                    <li><a class="dropdown-item  export-drop-item"
                                            href="{{ route('landlord.withdraw-method.csv') }}"><img
                                                src="{{ asset('backend/assets/img/icon/csv.svg') }}" />
                                            {{ __('Csv') }} </a></li>
                                    <li><a class="dropdown-item export-drop-item"
                                            href="{{ route('landlord.withdraw-method.excel') }}"><img
                    ...
                    <div class="delete-item delete-show d-none">
                        <div class="delete-item-show">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                                data-url="{{ route('landlord.withdraw-method.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    ...
                    <div class="responsibe-table">
                        <table class="table woodland-table">
                            ...
                            <tbody id="deposit-utilities-datas">
                                @include('landlord.withdraw-method.datas')
                            </tbody>
                        </table>
                    </div>
                    <div class="woodladn-pagination-wrapper">
                        <nav class="woodland-pagination">
                            <ul class="pagination">
                                <li class="page-item">{{ $withdraw_methods->links() }}</li>
                            </ul>
                        </nav>
                    </div>
                </div>
...
@push('modal')
    @include('landlord.withdraw-method.create')
    @include('landlord.withdraw-method.edit')
    @include('landlord.withdraw-method.payout-request')
    @include('landlord.withdraw-method.thankyou')
